// ignore_for_file: prefer_const_constructors, prefer_const_literals_to_create_immutables, non_constant_identifier_names, unused_local_variable, avoid_print, use_build_context_synchronously, unused_import, use_super_parameters, file_names
...
import 'package:jewellery_app/Login/ForgetPW.dart';
import 'package:jewellery_app/Login/Register.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
       actions: [
        // Sirf yeh IconButton yahan aayega, koi function nahi
        IconButton(
          onPressed: () {
            themeProvider.toggleTheme();
          },
          icon: Icon(
            // Agar Dark mode hai to Sun dikhao, nahi to Moon
            themeProvider.isDarkMode ? Icons.wb_sunny : Icons.nightlight_round,
            color: themeProvider.isDarkMode ? Colors.orange : null, // Optional styling
          ),
        ),
      ],
        // backgroundColor: Colors.black,
  ...
  Future<void> setprefs1(Map<String, dynamic> data) async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
...
Future<void> _signInWithGoogle(BuildContext context) async {
  try {
    ...
    if (googleUser == null) {
      ...
      return; // Early return if the user cancels the sign-in
    }
    ...
    if (_user != null) {
      await checkUser(_user!); // Check user in your database
      await _updateUserStatus(_user!.uid, true); // Update logged in status
      // await saveUserFcmToken(_user!.uid); // Save FCM token to Firestore if needed

      // Navigate to the next page after everything is done
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => CurveNavBar()), // Replace with your next page
      );
    }
  } catch (e) {
...
Future<void> _updateUserStatus(String userId, bool isLoggedIn) async {
  ...
  Future<void> checkUser(User user) async {
    try {
      ...
      var adminDoc = await _firestore.collection("admin").doc(user.uid).get();
      if (adminDoc.exists) {
        var adminData = adminDoc.data() as Map<String, dynamic>;
        await setprefs1(adminData); // Save preferences
        ...
        if (userDoc.exists) {
          ...
          await setprefs1(userData); // Save preferences
          Get.snackbar("SignIn", "User Signed In Successfully",
              // colorText: Colors.white
              );
           return Get.offAll(CurveNavBar()) ;
          }
...
class _SignInFormState extends State<SignInForm> {
  ...
  try {
    ...
    if (adminDoc.exists) {
      ...
      // await saveUserFcmToken(user.uid); // Save FCM token for admin
      Get.snackbar("SignIn", "Admin Signed In Successfully",
          // colorText: Colors.white
          );
      Get.offAll(AdminDashboard());
      ...
      if (userDoc.exists) {
        ...
        // await saveUserFcmToken(user.uid); // Save FCM token for user
        Get.snackbar("SignIn", "User Signed In Successfully",
            // colorText: Colors.white
            );
        ...
        await _updateUserStatus(user.uid, false); // Update status to logged out
...
Future<void> _updateUserStatus(String userId, bool isLoggedIn) async {
  ...
  void emailandpassword(BuildContext context) {
  ...
  bool isValidEmail(String email) {
    ...
    return validEmail.any((domain) => email.endsWith(domain));
  }
